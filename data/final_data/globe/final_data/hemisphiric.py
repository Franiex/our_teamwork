...
# Create folders in the same directory as script
os.makedirs('./table_and_graph', exist_ok=True)
...
plt.tight_layout()
plt.savefig('./table_and_graph/hemisphere_temperature_comparison.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.grid(True, alpha=0.3, linestyle='--')
plt.tight_layout()
plt.savefig('./table_and_graph/hemisphere_temperature_difference.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('./table_and_graph/warming_rates_by_period.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.grid(True, alpha=0.3, linestyle='--')
plt.tight_layout()
plt.savefig('./table_and_graph/temperature_variability.png', dpi=300, bbox_inches='tight')
plt.close()
...
comparison_df.to_csv('./table_and_graph/hemisphere_comparison_table.csv', index=False)
...
with open('./analyse/hemispheric_analysis_report.txt', 'w', encoding='utf-8') as f:
    f.write(report)
...
combined_export.to_csv('./analyse/processed_hemisphere_data.csv', index=False)
...
metrics_df.to_csv('./analyse/key_metrics_summary.csv', index=False)
