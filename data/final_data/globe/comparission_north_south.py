...
os.makedirs('./final_data/region', exist_ok=True)
os.makedirs('./final_data/Analyse', exist_ok=True)

csv_files = glob.glob('*.csv')
if not csv_files:
    csv_files = glob.glob('south_and_north*.csv')
if not csv_files:
...
plt.tight_layout()
plt.savefig('./final_data/region/hemisphere_temperature_comparison.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.grid(True, alpha=0.3, linestyle='--')
plt.tight_layout()
plt.savefig('./final_data/region/hemisphere_temperature_difference.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('./final_data/region/warming_rates_by_period.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.grid(True, alpha=0.3, linestyle='--')
plt.tight_layout()
plt.savefig('./final_data/region/temperature_variability.png', dpi=300, bbox_inches='tight')
plt.close()
...
comparison_df.to_csv('./final_data/region/hemisphere_comparison_table.csv', index=False)
...
print("\nOutputs created:")
print("  ./final_data/region/ - 4 graphs + comparison table")
print("  ./final_data/Analyse/ - report + processed data")